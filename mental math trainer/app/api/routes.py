...
def resolve_settings(payload: Optional[GameStartRequest]) -> GenerationSettings:
    ...
    elif payload is not None and payload.mode == "zetamac_optimization":
        settings = GenerationSettings.from_preset(
            payload.preset_name or DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME,
            mode=payload.mode,
        )
    ...
    if payload is not None:
        ...
        if payload.auto_advance_on_correct is not None:
            settings.auto_advance_on_correct = payload.auto_advance_on_correct
        if payload.wrong_answer_penalty is not None:
            settings.wrong_answer_penalty = payload.wrong_answer_penalty
...
def export_history(request: Request) -> dict[str, object]:
    storage = request.app.state.storage
    return storage.export_history()


@router.get("/history/recent")
def recent_history(request: Request, limit: int = Query(default=20, ge=1, le=100)) -> dict[str, object]:
    storage = request.app.state.storage
    return {"items": storage.recent_runs(limit=limit)}
