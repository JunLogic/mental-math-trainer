...
class SessionManager:
    ...
    def start_session(self, settings: GenerationSettings) -> GameSession:
        ...
        session = GameSession(
            ...
            attempted=0,
            questions=questions,
            generation_settings=self._session_generation_settings(settings, generator),
    ...
    def answer_question(
        ...
        with self.lock:
            ...
            if is_correct:
                ...
                session.incorrect += 1
                if session.mode not in {"zetamac", "zetamac_optimization"}:
                    session.score -= 1
    ...
    def _evaluate_answer(
        ...
        if session.mode in {"zetamac", "zetamac_optimization"}:
    ...
    def _session_generation_settings(
        self,
        settings: GenerationSettings,
        generator: QuestionGenerator,
    ) -> dict[str, object]:
        generation_settings = settings.to_dict()
        policy_summary = generator.runtime_policy_summary()
        if policy_summary:
            generation_settings["runtime_generation_policy"] = policy_summary
    ...
    def build_session_payload(self, session: GameSession, include_history: bool = False) -> dict[str, object]:
        ...
        if (
            ...
            and session.mode in {"assessment", "zetamac", "zetamac_optimization"}
            and bool(session.generation_settings.get("auto_advance_on_correct", False))
        ):
            payload["question"]["auto_advance_match_answer"] = current_question.correct_answer

        latest_answer = None
        if session.current_index > 0 and session.current_index <= len(session.questions):
            previous_question = session.questions[session.current_index - 1]
            if previous_question.result in {"correct", "incorrect"}:
                latest_answer = {
                    "question_number": session.current_index,
                    "result": previous_question.result,
                    "submitted_answer": previous_question.submitted_answer,
