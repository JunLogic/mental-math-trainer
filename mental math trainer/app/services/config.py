...
DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME = "zetamac_optimization_default"
...
PRESET_ALIASES: dict[str, str] = {
    ...
    "zetamac_optimization": DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME,
}
...
PRESET_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME: {
        ...
        "weights": {
            "addition_weight": 30,
            "subtraction_weight": 28,
            "multiplication_weight": 22,
            "division_weight": 20,
            "decimal_weight": 0,
            "missing_variable_weight": 0,
        },
        "difficulty_profile": BALANCED_DIFFICULTY,
        "settings": {
            "duration_seconds": DEFAULT_ZETAMAC_DURATION,
            "max_questions": DEFAULT_ZETAMAC_DURATION * 3,
        },
    },
}
...
DEFAULT_ZETAMAC_OPTIMIZATION_SETTINGS: dict[str, Any] = {
    "duration_seconds": DEFAULT_ZETAMAC_DURATION,
    "allow_missing_variable": False,
    "artifact_preferred_view": "correct_only",
    "use_weakness_artifacts": True,
    "operations": {
        "addition": True,
        "subtraction": True,
        "multiplication": True,
    ...
    "targeted_min_difficulty": 68.0,
    "related_min_difficulty": 48.0,
    "rhythm_max_difficulty": 34.0,
    "max_target_streak": 2,
}
...
def validate_zetamac_optimization_settings(raw_settings: dict[str, Any]) -> dict[str, Any]:
    settings = deep_merge_dicts(DEFAULT_ZETAMAC_OPTIMIZATION_SETTINGS, raw_settings)
    duration_seconds = int(settings["duration_seconds"])
    if duration_seconds not in ZETAMAC_ALLOWED_DURATIONS:
        raise ValueError("zetamac optimization duration_seconds must be one of 30, 60, 120, 300, or 600.")

    operations = settings.get("operations", {})
    if not any(bool(operations.get(operation)) for operation in ("addition", "subtraction", "multiplication", "division")):
        raise ValueError("At least one Zetamac optimization operation must be enabled.")
    settings["operations"] = {
        "addition": bool(operations.get("addition", True)),
        "subtraction": bool(operations.get("subtraction", True)),
        "multiplication": bool(operations.get("multiplication", True)),
        "division": bool(operations.get("division", True)),
    }

    raw_route_weights = settings.get("route_weights", {})
    route_weights = {
        "targeted": max(0.0, float(raw_route_weights.get("targeted", 0.55))),
        "related": max(0.0, float(raw_route_weights.get("related", 0.25))),
        "rhythm": max(0.0, float(raw_route_weights.get("rhythm", 0.20))),
    }
    if sum(route_weights.values()) <= 0:
        raise ValueError("At least one Zetamac optimization route weight must be greater than zero.")

    settings["duration_seconds"] = duration_seconds
    settings["allow_missing_variable"] = bool(settings.get("allow_missing_variable", False))
    settings["artifact_preferred_view"] = str(settings.get("artifact_preferred_view", "correct_only"))
    settings["use_weakness_artifacts"] = bool(settings.get("use_weakness_artifacts", True))
    settings["route_weights"] = route_weights
    settings["targeted_min_difficulty"] = clamp(float(settings.get("targeted_min_difficulty", 68.0)), 0.0, 100.0)
    settings["related_min_difficulty"] = clamp(float(settings.get("related_min_difficulty", 48.0)), 0.0, 100.0)
    settings["rhythm_max_difficulty"] = clamp(float(settings.get("rhythm_max_difficulty", 34.0)), 0.0, 100.0)
    settings["max_target_streak"] = max(1, int(settings.get("max_target_streak", 2)))
    return settings
...
class GenerationSettings:
    ...
    zetamac_optimization_settings: dict[str, Any] = field(
        default_factory=lambda: copy.deepcopy(DEFAULT_ZETAMAC_OPTIMIZATION_SETTINGS)
    )
    adaptive_enabled: bool = False
    target_pace_seconds: float = DEFAULT_ADAPTIVE_TARGET_PACE
    initial_difficulty: float = DEFAULT_ADAPTIVE_INITIAL_DIFFICULTY
    auto_advance_on_correct: bool = False
    wrong_answer_penalty: bool = False
    ...
    def validate(self) -> None:
        ...
        if self.mode not in {"assessment", "training", "zetamac", "zetamac_optimization"}:
            raise ValueError("mode must be 'assessment', 'training', 'zetamac', or 'zetamac_optimization'.")
        ...
        self.auto_advance_on_correct = bool(self.auto_advance_on_correct) and self.mode in {
            "assessment",
            "zetamac",
            "zetamac_optimization",
        }
        self.wrong_answer_penalty = bool(self.wrong_answer_penalty) and not self.auto_advance_on_correct
        self.zetamac_settings = validate_zetamac_settings(self.zetamac_settings)
        self.zetamac_optimization_settings = validate_zetamac_optimization_settings(self.zetamac_optimization_settings)
        ...
        if self.mode == "zetamac_optimization":
            self.duration_seconds = self.zetamac_optimization_settings["duration_seconds"]
            self.max_questions = estimate_zetamac_question_budget(self.duration_seconds)
            self.decimal_weight = 0
            if not self.zetamac_optimization_settings["allow_missing_variable"]:
                self.missing_variable_weight = 0
    ...
    def from_dict(cls, values: dict[str, Any]) -> "GenerationSettings":
        ...
        elif values.get("mode") == "zetamac_optimization" or preset_name == DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME:
            base = cls.from_preset(DEFAULT_ZETAMAC_OPTIMIZATION_PRESET_NAME)
        else:
