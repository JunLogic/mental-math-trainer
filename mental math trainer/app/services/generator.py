...
from app.services.config import (
    ...
    GenerationSettings,
    clamp,
    difficulty_profile_for_scalar,
)
from app.services.feature_extraction import extract_saved_question_features
from app.services.zetamac_optimization import TrainingTarget, WeaknessArtifactBundle, WeaknessArtifactLoader
...
class QuestionGenerator:
    def __init__(
        ...
        seed: int,
        weakness_loader: WeaknessArtifactLoader | None = None,
    ) -> None:
        ...
        self.weakness_loader = weakness_loader
        self.optimization_bundle = self._load_zetamac_optimization_bundle()
        self.optimization_recent_routes: deque[str] = deque(maxlen=4)
        self.optimization_recent_targets: deque[str] = deque(maxlen=4)
    ...
    def runtime_policy_summary(self) -> dict[str, Any]:
        if self.settings.mode != "zetamac_optimization":
            return {}

        optimization_settings = self.settings.zetamac_optimization_settings
        enabled_operations = [
            operation
            for operation, enabled in optimization_settings["operations"].items()
            if enabled
        ]
        bundle = self.optimization_bundle or WeaknessArtifactBundle(
            status="disabled",
            artifact_path=None,
            artifact_source=None,
            weakness_weighting_applied=False,
            reverse_problems_excluded=True,
            missing_variable_allowed=False,
            selected_view=None,
            available_views=(),
            targets=(),
            rejected_slice_keys=(),
        ...
        summary.update(
            {
                "policy_name": "zetamac_optimization_v1",
                "forward_only": True,
                "integer_only": True,
                "enabled_operations": enabled_operations,
                "route_weights": dict(optimization_settings["route_weights"]),
                "max_target_streak": optimization_settings["max_target_streak"],
                "allow_missing_variable": optimization_settings["allow_missing_variable"],
                "targeted_min_difficulty": optimization_settings["targeted_min_difficulty"],
                "related_min_difficulty": optimization_settings["related_min_difficulty"],
                "rhythm_max_difficulty": optimization_settings["rhythm_max_difficulty"],
            }
        )
        return summary
    ...
    def _generate_question(self, adaptive_difficulty: float | None = None) -> QuestionRecord:
        ...
        if self.settings.mode == "zetamac_optimization":
            return self._generate_zetamac_optimization_question(adaptive_difficulty=adaptive_difficulty)
        ...
        return self._build_standard_question(
    ...
    def _load_zetamac_optimization_bundle(self) -> WeaknessArtifactBundle | None:
        if self.settings.mode != "zetamac_optimization":
            return None

        optimization_settings = self.settings.zetamac_optimization_settings
        if not optimization_settings.get("use_weakness_artifacts", True):
            return WeaknessArtifactBundle(
                ...
                weakness_weighting_applied=False,
                reverse_problems_excluded=True,
                missing_variable_allowed=bool(optimization_settings["allow_missing_variable"]),
                selected_view=None,
                available_views=(),
                targets=(),
                rejected_slice_keys=(),
        ...
        loader = self.weakness_loader or WeaknessArtifactLoader()
        enabled_operations = [
            operation
            for operation, enabled in optimization_settings["operations"].items()
            if enabled
        ]
        return loader.load_forward_targets(
            allow_missing_variable=bool(optimization_settings["allow_missing_variable"]),
            included_operations=enabled_operations,
            preferred_view=str(optimization_settings["artifact_preferred_view"]),
        )

    def _build_standard_question(
        self,
        *,
        ...
        adaptive_difficulty: float | None,
    ...
    def _generate_zetamac_optimization_question(self, adaptive_difficulty: float | None = None) -> QuestionRecord:
        route = self._choose_optimization_route()
        target: TrainingTarget | None = None
        question: QuestionRecord | None = None

        if route in {"targeted", "related"}:
            target = self._select_optimization_target()
            if target is not None:
                question = self._generate_targeted_optimization_question(
                    target=target,
                    route=route,
                    adaptive_difficulty=adaptive_difficulty,
                )
        ...
        if question is None:
            route = "rhythm"
            question = self._generate_optimization_rhythm_question(adaptive_difficulty=adaptive_difficulty)

        bundle = self.optimization_bundle
        question.metadata.update(
            {
                "generation_route": route,
                "generation_target_slice_key": target.slice_key if target is not None else None,
                "generation_target_description": target.slice_description if target is not None else None,
                "weakness_weighting_applied": bool(bundle and bundle.weakness_weighting_applied),
                "reverse_problems_excluded": True,
                "missing_variable_allowed": bool(self.settings.zetamac_optimization_settings["allow_missing_variable"]),
                "artifact_status": bundle.status if bundle is not None else "disabled",
                "artifact_path": bundle.artifact_path if bundle is not None else None,
            }
        )
        self.optimization_recent_routes.append(route)
        if target is not None:
            self.optimization_recent_targets.append(target.slice_key)
        elif route == "rhythm":
            self.optimization_recent_targets.append("rhythm")
        return question

    def _choose_optimization_route(self) -> str:
        optimization_settings = self.settings.zetamac_optimization_settings
        bundle = self.optimization_bundle
        has_targets = bool(bundle and bundle.weakness_weighting_applied and bundle.targets)
        if not has_targets:
            return "rhythm"

        max_target_streak = int(optimization_settings["max_target_streak"])
        if len(self.optimization_recent_routes) >= max_target_streak and all(
            route == "targeted"
            for route in list(self.optimization_recent_routes)[-max_target_streak:]
        ):
            return "rhythm"

        route_weights = dict(optimization_settings["route_weights"])
        if self.optimization_recent_routes:
            last_route = self.optimization_recent_routes[-1]
            if last_route == "targeted":
                route_weights["targeted"] *= 0.55
                route_weights["rhythm"] *= 1.35
            elif last_route == "related":
                route_weights["rhythm"] *= 1.15
    ...
    def _select_optimization_target(self) -> TrainingTarget | None:
        bundle = self.optimization_bundle
        if bundle is None or not bundle.targets:
            return None

        targets = list(bundle.targets)
        if len(targets) > 1 and len(self.optimization_recent_targets) >= 2:
            recent_targets = list(self.optimization_recent_targets)[-2:]
            if recent_targets[0] == recent_targets[1]:
                alternatives = [target for target in targets if target.slice_key != recent_targets[0]]
                if alternatives:
                    targets = alternatives

        return self.rng.choices(
            population=targets,
            weights=[max(float(target.priority_score), 0.01) for target in targets],
            k=1,
        )[0]

    def _generate_targeted_optimization_question(
        self,
        *,
        ...
        difficulty_profile = self._optimization_difficulty_profile(route, adaptive_difficulty)
        max_attempts = 72 if route == "targeted" else 28
        used_missing_variable = bool(self.settings.zetamac_optimization_settings["allow_missing_variable"]) and any(
            feature == "used_missing_variable" and bool(value)
            for feature, value in target.filters
        )

        for _ in range(max_attempts):
            candidate = self._build_standard_question(
                base_operation=target.base_operation or self._weighted_forward_operation_choice(),
                used_decimal=False,
                used_missing_variable=used_missing_variable,
                difficulty_profile=difficulty_profile,
                adaptive_difficulty=adaptive_difficulty,
            )
            if self._question_matches_target(candidate, target):
                return candidate
        ...
        return self._build_standard_question(
            ...
            used_decimal=False,
            used_missing_variable=False,
            difficulty_profile=self._optimization_difficulty_profile("related", adaptive_difficulty),
            adaptive_difficulty=adaptive_difficulty,
        )

    def _generate_optimization_rhythm_question(self, adaptive_difficulty: float | None = None) -> QuestionRecord:
        return self._build_standard_question(
            base_operation=self._weighted_forward_operation_choice(),
            used_decimal=False,
            used_missing_variable=False,
            difficulty_profile=self._optimization_difficulty_profile("rhythm", adaptive_difficulty),
            adaptive_difficulty=adaptive_difficulty,
        )

    def _weighted_forward_operation_choice(self) -> BaseOperation:
        optimization_settings = self.settings.zetamac_optimization_settings
        enabled_operations = [
            operation
            for operation, enabled in optimization_settings["operations"].items()
        ...
        operation_weights = {
            operation: self.operation_weights[operation]
            for operation in enabled_operations
        }
        return self.rng.choices(
            population=list(operation_weights.keys()),
            weights=list(operation_weights.values()),
            k=1,
        )[0]

    def _optimization_difficulty_profile(
        self,
        route: str,
        adaptive_difficulty: float | None,
    ) -> dict[str, Any]:
        optimization_settings = self.settings.zetamac_optimization_settings
        if route == "targeted":
            minimum = float(optimization_settings["targeted_min_difficulty"])
            if adaptive_difficulty is None:
                return HARD_DIFFICULTY
            return difficulty_profile_for_scalar(max(minimum, adaptive_difficulty))

        if route == "related":
            minimum = float(optimization_settings["related_min_difficulty"])
            if adaptive_difficulty is None:
                return BALANCED_DIFFICULTY
            return difficulty_profile_for_scalar(max(minimum, adaptive_difficulty))

        maximum = float(optimization_settings["rhythm_max_difficulty"])
        if adaptive_difficulty is None:
            return EASY_DIFFICULTY
        return difficulty_profile_for_scalar(min(maximum, adaptive_difficulty))

    def _question_matches_target(self, question: QuestionRecord, target: TrainingTarget) -> bool:
        features = self._generated_question_features(question)
        for feature_name, expected_value in target.filters:
            actual_value = features.get("operation" if feature_name == "base_operation" else feature_name)
    ...
    def _generated_question_features(self, question: QuestionRecord) -> dict[str, Any]:
        return extract_saved_question_features(
            {
                "base_operation": question.base_operation,
                "used_decimal": question.used_decimal,
                "used_missing_variable": question.used_missing_variable,
                "answer_role": question.metadata.get("answer_role"),
                "left": question.metadata.get("left"),
                "right": question.metadata.get("right"),
                "result": question.metadata.get("result"),
