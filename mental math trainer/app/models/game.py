...
MODE_LABELS: dict[str, str] = {
    ...
    "Zetamac Optimization": "Zetamac Optimization",
}
...
class QuestionRecord:
    ...
    def to_public(self, question_number: int, total_questions: int, mode: str) -> dict[str, Any]:
        payload = {
            ...
            "generation_route": self.metadata.get("generation_route"),
            "generation_target_slice_key": self.metadata.get("generation_target_slice_key"),
            "weakness_weighting_applied": self.metadata.get("weakness_weighting_applied"),
...
class GameSession:
    ...
    def operation_mix_used(self) -> str:
        ...
        if self.mode == "zetamac_optimization":
            operations = self.generation_settings.get("zetamac_optimization_settings", {}).get("operations", {})
            enabled = [name for name, enabled in operations.items() if enabled]
            return ", ".join(display_operation_name(name) for name in enabled) if enabled else "None"
